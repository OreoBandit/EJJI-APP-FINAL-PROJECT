...
router.post('/registerUser', (req, res) => {
  const data = req.body
  const q = `insert into users (username, u_password, email)
              values ('${data.username}', '${data.u_password}', '${data.email}')`
...
router.post('/login', (req, res) => {
  data = req.body
  q = `select * from users where email = '${data.email}' and u_password = '${data.u_password}'`
  database.query(q, (error, result) => {
    if (error) {
      throw error
    }
